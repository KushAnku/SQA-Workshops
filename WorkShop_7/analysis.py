...
def trackTaint(val2track, df_list_param): 
    ...
    # Initializing the taint tracking flow with the initial value
    taint_flow = str(val2track)
    current_taint = str(val2track)

    # Looking for the initial value (1000) in the assignments
    for index, row in var_.iterrows():
        if str(row['RHS']) == current_taint:
            current_taint = row['LHS']
            taint_flow += f" -> {current_taint}"

    # Tracking the function argument (val1 -> v1)
    for index, row in call_.iterrows():
        if row['ARG_NAME'] == current_taint:
            current_taint = "v1"
            taint_flow += f" -> {current_taint}"

    # Tracking  the result of the function (v1 -> res)
    for index, row in func_var.iterrows():
        rhs_values = str(row['RHS']).split(',')
        if "v1" in rhs_values and row['LHS'] == "res":
            current_taint = "res"
            taint_flow += f" -> {current_taint}"
